...
public class ExtentReportsUtility {
	...
	public void startExtentReport() {
		...
		report.setSystemInfo("Host Name","Salesforce Application");
		report.setSystemInfo("Environment","QA");
		report.setSystemInfo("user Name","Dharani Reddy");
		spark.config().setDocumentTitle("Test Execution Report");
		spark.config().setReportName("firebaseregression tests");
	...
	public void endreport() {
		...
		public void logTestFailedWithScreenshot(String path) {
		    try {
		        testLogger.fail("Screenshot",
		                MediaEntityBuilder.createScreenCaptureFromPath(path).build());
		    } catch (Exception e) {
		        testLogger.fail("Failed to attach screenshot: " + e.getMessage());
		    }
		}
}
